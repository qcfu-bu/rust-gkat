...
let print g f x = 
  Format.pp_print_list ~pp_sep:(fun f () -> Format.pp_print_char f ',') g f (elements x)
...
let rec equal x y = match x,y with 
  | Empty,Empty -> true
  | Leaf x,Leaf y -> x==y
  | Branch(i,j,l,r),Branch(i',j',l',r') -> i=i' && j=j' && equal l l' && equal r r'
  | _ -> false

let rec hash = function
  | Empty -> 0
  | Leaf x -> x.hkey
  | Branch(_,_,l,r) -> hash l + hash r
